...
import {
  ...
  updateMeetingTypeFn,
  deleteMeetingTypeFn,
  type MeetingTypeWithDetails,
} from "@/server/meetingTypes";
...
export const Route = createFileRoute(
  ...
  loader: async ({ params }): Promise<MeetingTypeWithDetails> => {
    ...
    const mt = await (getMeetingTypeFn as unknown as (opts: { data: { meetingTypeId: string } }) => Promise<MeetingTypeWithDetails | null>)({ data: { meetingTypeId } });
...
function mapLoaderDataToState(
  mt: NonNullable<Awaited<ReturnType<typeof getMeetingTypeFn>>>
) {
  return {
    name: mt.name,
    ...
    sections: mt.sections.map((s) => ({
      ...
      freeformBlocks: s.freeformBlocks.map((b) => ({
        label: b.label,
...
function EditMeetingTypePage() {
  ...
  const data = mapLoaderDataToState(loaderData);

  const [name, setName] = useState(data.name);
  const [description, setDescription] = useState(data.description);
  const [sections, setSections] = useState<SectionState[]>(data.sections);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await (updateMeetingTypeFn as unknown as (opts: {
        data: { meetingTypeId: string; data: typeof payload };
      }) => Promise<{ error?: string }>)({ data: { meetingTypeId, data: payload } });
      if (result.error) {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const result = await (deleteMeetingTypeFn as unknown as (opts: { data: { meetingTypeId: string } }) => Promise<{ error?: string }>)({ data: { meetingTypeId } });
      if (result.error) {
        setError(result.error);
        return;
      }
          ...
          <button
            ...
            className={
              deleteConfirm
                ...
                : "px-4 py-2 border border-zinc-300 rounded-md hover:bg-zinc-100 disabled:opacity-50"
            }
          >
            {deleteConfirm ? "Click again to confirm delete" : "Delete meeting type"}
          </button>
        </div>
      </form>
    </div>
